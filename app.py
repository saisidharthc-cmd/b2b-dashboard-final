...
@st.cache_data
def load_data():
    # Try to load from same directory (for GitHub / deployment)
    possible_paths = [
        "B2B_Order_Dataset.xlsx",
        os.path.join(os.path.dirname(__file__), "B2B_Order_Dataset.xlsx"),
        "/mnt/user-data/uploads/B2B_Order_Dataset.xlsx",
    ]
    df = None
    ...
    if df is None:
        st.error("⚠️ Dataset not found. Please ensure `B2B_Order_Dataset.xlsx` is in the same folder as `app.py`.")
        st.stop()
